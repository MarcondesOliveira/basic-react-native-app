...
function Form(){
  ...
  function handleButtonPress() { 
    axios.post('http://192.168.1.107:3031/cadastro', {nome, idade, estado})
      .then(response => {
  ...
  useEffect(() => {
    axios.get('http://192.168.1.107:3030/estados').then(response => {
      setEstados(response.data.map(estado => ({label: estado.uf, key: estado.uf, value: estado.uf})))
    })
  }, [])
 
  const placeholder = { label:'Selecione o estado', value: '', color: 'red'}; 
